        ...
        .then(
          ([
            ...
            const webTileLayer = new WebTileLayer({
              urlTemplate:
                "http://{subDomain}.tianditu.gov.cn/vec_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=vec&STYLE=default&TILEMATRIXSET=c&FORMAT=tiles&TILEMATRIX={level}&TILEROW={row}&TILECOL={col}&tk=94a9c789edc5d02e50a129a2276d3ab7",
    ...
    eventHandler: function (event) {
      ...
      this.mapConfig.mapView.hitTest(event, opts).then(function (response) {
        ...
        if (response.results[0].graphic.geometry.type === "point") {
          ...
          //将获得的结果填充到表格中
          _self.pointTitle =
            "锦石岩-长老峰景区路线——" + pointGraphic.attributes.name;
          _self.desserts = [];
