...
export default {
  ...
  data() {
    return {
      ...
      lackTip: false, //缺少点的提示信息
      errorTip: false, //路径分析没有结果的提示信息
      pointType: 1, //设置停靠点或者障碍点
      routeLayer: null, //路径分析结果图层
      stopSymbol: null, //停靠点样式
      barrierSymbol: null, //障碍点样式
      routeSymbol: null, //返回的路线样式
      routeParams: null, //查询参数
      routeTask: null, //路径导航的服务
      addPointsL: null, //地图绘制点的监听器
    };
  },
  ...
  methods: {
      ...
      loadModules(
        ...
        .then(
          ([
            ...
            //路径分析的结果图层
            _self.routeLayer = new GraphicsLayer();
            //原始路网图层
         ...
         // 范围图层
            ...
            //查询参数
            _self.routeParams = new RouteParameters({
    ...
    addListening: function () {
      this.addPointsL = this.mapConfig.mapView.on("click", this.addPoints);
    },
    ...
    addPoints: function (event) {
      ...
      loadModules(["esri/Graphic"], option).then(([Graphic]) => {
        if (_self.pointType == 1) {
          // 将停靠点添加在地图上
          var stop = new Graphic({
            geometry: event.mapPoint,
            symbol: _self.stopSymbol,
          });
          _self.routeLayer.add(stop);
          _self.routeParams.stops.features.push(stop);
        }
        if (_self.pointType == 2) {
          //将障碍点添加到地图上
          var barrier = new Graphic({
            geometry: event.mapPoint,
            symbol: _self.barrierSymbol,
          });
          _self.routeLayer.add(barrier);
          _self.routeParams.pointBarriers.features.push(barrier);
        }
      });
    },
    ...
    solveResults: function () {
      ...
      if (_self.routeParams.stops.features.length < 2) {
        _self.lackTip = true;
        return;
      }
      if (_self.routeParams.stops.features.length >= 2) {
        console.log(_self.routeParams);
        // 进行查询
        _self.routeTask
          .solve(_self.routeParams)
          .then(_self.showRoute, _self.logError);
      }
    },
    ...
    //清空数据
    cleanAll: function () {
      this.addPointsL.remove();
      this.routeLayer.removeAll();
      this.routeParams.stops.features.length = 0; //清空参数
      this.routeParams.pointBarriers.features.length = 0; //清空参数
    },
  },
  mounted: function () {
    NProgress.start();
